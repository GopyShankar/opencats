...
class ActivityDataGridOfferMail extends DataGrid
{
    protected $_siteID;
    ...
    {   
        ...
        {
            $this->dateCriterion .= ' AND activity_offermail.date_created >= ' . $parameters['period'] . ' ';
        }
        else
        {
            if (isset($parameters['startDate']) && !empty($parameters['startDate']))
            {
                $this->dateCriterion .= ' AND activity_offermail.date_created >= \'' .$parameters['startDate'].'\' ';
            }
            
            if (isset($parameters['endDate']) && !empty($parameters['endDate']))
            {
                $this->dateCriterion .= ' AND activity_offermail.date_created <= \''.$parameters['endDate'].'\' ';
            }
        }
        ...
        $this->_classColumns = array( 
            'Date' =>           array('pagerRender'    => 'return $rsData[\'dateCreated\'];', 
                                      ...
                                      'filter' => 'activity_offermail.date_created'),
             ...
             'Mail Address' =>      array('pagerRender'    => '$ret = $rsData[\'mail_address\']; return $ret;', 
                                     ...
                                     'pagerOptional'   => true,
                                     'alphaNavigation' => true,
                                     'filter'          => 'activity_offermail.mail_address'),        
             ...
             'Notes' =>      array('pagerRender'    => 'return $rsData[\'notes\'];', 
                                     ...
                                     'pagerOptional'   => true,
                                     'alphaNavigation' => true,
                                     'filter'    => 'activity_offermail.notes'),
    ...
    {   
        $sql = sprintf(
            "SELECT SQL_CALC_FOUND_ROWS %s
                activity_offermail.activity_offermail_id AS activityMailID,
                activity_offermail.site_id AS siteID,
                activity_offermail.mail_address AS mail_address,
                activity_offermail.notes AS notes,
                activity_type.short_description AS typeDescription,
                DATE_FORMAT(
                    activity_offermail.date_created, '%%m-%%d-%%y (%%h:%%i %%p)'
                ) AS dateCreated,
                activity_offermail.date_created AS dateCreatedSort,
            ...
            LEFT JOIN user AS entered_by_user
                ON activity_offermail.entered_by = entered_by_user.user_id
            LEFT JOIN activity_type
                ON activity_offermail.type = activity_type.activity_type_id
            WHERE
                activity_offermail.site_id = %s
                %s
                %s
            UNION
            SELECT %s
